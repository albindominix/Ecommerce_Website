...
const Products = () => {
  ...
  const performAPICall = async () => {
    setIsLoading(true);
    try {
      const GET_API_LINK = config["endpoint"] + "/products";
      const response = await axios.get(GET_API_LINK);
      setIsLoading(false);
  ...
  useEffect(() => {
    fetchCart(token)
      .then((cartData) => generateCartItemsFrom(cartData, productData))
  ...
  const addToCart = async (
    ...
    try {
      ...
      const response = await axios.post(cART_POST_API,
        {productId,qty},
        {
          headers:{
            Authorization: `Bearer ${token}`,
      ...
      <Grid container>
        ...
        {token ? (
          <Grid item xs={12} md={3} bgcolor="#E9F5E1">
            <Cart items={items} products={productData} handleQuantity={addToCart}/>
          </Grid>
        ) : null}
      </Grid>
      <Footer />
