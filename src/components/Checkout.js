...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const response = await axios.post(ADDRESS_POST_API,{address:newAddress.value},{
        headers : {
          Authorization : `Bearer ${token}`
        }
      })
  ...
  const validateRequest = (items, addresses) => {
    ...
    const payableCost =items.map((item)=>(item["cost"]*item["qty"]))
    const totalPayableCost= payableCost.reduce((total,value)=>total+value)
    //const enoughBalance=balance-totalPayableCost;
    ...
    else if(balance < totalPayableCost){
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      const Checkout_POST_API=`${config["endpoint"]}/cart/checkout`;
      const response=await axios.post(Checkout_POST_API,{addressId:addresses.selected},{
        headers:{
          Authorization: `Bearer ${token}`
        }
      });
